...
after the British mathematician, George Boole, who first formulated *Boolean algebra* 
--- some rules for reasoning about and combining these values.
This is the basis of all modern computer logic.

一個「布林值」也叫做「真假值」，
在英文裡面其實就是 Yes/No 或是 True/False。
取名為「布林」是為了紀念英國數學家喬治布林，
他是第一個制定一套真假值邏輯推理方法的數學家。
他的某些理論甚至成為現代電腦科學的基礎。
...
在 Python 程式語言中，選擇使用 **True** 和 **False** (特別注意開頭大寫字母)來代表 真或假。
這 2 個值自己字成一中資料型態，就叫做 **bool** 資料型，
與 **int**, **float**, **str** 一起成為 Python 程式語言的核心資料型。
...
一個**布林式子**可以產生一個**布林值**，就是真假值，Yes/No, 是/否, 對/錯，
一般就是我們邏輯思考判斷的結果。
舉例來說，雙等號 ``==`` 這個運算子可以測試 2 個值 **是否** 相等。
測試結果就是 True/False 二者選一。 
請看以下例子：
...
And now we'll add two lines to fill each bar.  Our final program now looks like this:
    ...
    .. sourcecode:: python3
        ...
        def 畫長條圖(烏龜, 高):
            """ 叫 烏龜 依照 高，畫長條圖 """
            烏龜.begin_fill()           # 開始填滿顏色
            烏龜.left(90)               # 左轉 90 度
            烏龜.forward(高)            # 前進 高度的數字
            烏龜.write("  "+ str(高))   # 寫下高度的數字
            烏龜.right(90)              # 右轉 90 度
            烏龜.forward(40)            # 前進 40 步
            烏龜.right(90)              # 右轉 90 度
            烏龜.forward(高)            # 前進 高度的數字
            烏龜.left(90)               # 左轉 90 度
            烏龜.end_fill()             # 結束填滿顏色
            烏龜.forward(10)            # 前進 10 步    
        ...
        高度列表= [48,117,200,240,160,260,220]  # 用一個列表儲存所有高度的值。
