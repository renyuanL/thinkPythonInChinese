...
計算機經常被用來自動執行重複性的任務。
重複相同或類似的任務卻能不犯錯誤，這點電腦做的很好， 
而人類反而做不好。 

一組語句重複執行被稱為 **迭代** 。因為 
迭代是如此普遍，Python 提供幾種語言的特徵，使其 
更容易使用。我們在第 3 章已經看到了 ``for`` 引導出來的敘述。
這大概是最普遍的迭代形式。
在本章中我們要來看看 ``while`` 敘述---這是另一種方式讓你的 程式做迭代，但使用情況稍有不同。 
...
正如我們前面提到的，對同一個變數而言，多次指定不同的值給它，這是符合語法的。
一次新的指定會讓一個已存在的變數指向新的值，而不再指向舊的值
...
這是因為第一次 ``剩餘通話時間`` 印出時，其值為 15，而第二次 ``剩餘通話時間`` 印出時，其值已經重新指定為 7 了。
...
區分 **指定敘述** 和 用來測試「是否相等」的 **布林式子** 是很重要的。

 
因為 Python 使用「單等號」（ ``=`` ）於指定敘述中，
程式初學者很容易把 ``a = b`` 這種指定敘述解釋成布林式子。不像數學語言，它並非布林式子。請特別注意，Python 使用「雙等號」（ ``==`` ）於 布林式子中，
 
還要注意一點，一個用來測試「是否相等」的 **布林式子** 等號兩邊是對稱的，
但 **指定敘述** 則否。
例如， ``a == 7`` 與 ``7 == a`` 具一樣的意思，a 和 7 位置互換無所謂。
但是， ``a = 7`` 與 ``7 = a`` ，則有所不同，後者甚至不合語法。

翻譯本書的筆者個人的建議為，在作指定敘述時，「單等號」（ ``=`` ）永遠連著其左邊的變數名稱，而其右邊的值，永遠與其至少有一個以上空白隔開。
比如： ``a=  7``
...
在 Python 中，指定敘述可以使兩個變數相等，
但由於進一步的指定敘述可以改變兩個變數中的任何一個， 
他們不必然一直保持相等：
    ...
    .. sourcecode:: python3
        ...
        甲= 5
        乙= 甲    # 程式跑到此行之後， 甲, 乙 二者相等
        甲= 3     # 但跑到此行之後，   甲, 乙 二者就不再相等了
