...
.. module:: 龜模組
   :synopsis: 中文 龜圖 給小孩玩的電腦程式畫圖。
.. sectionauthor:: 呂仁園 <renyuan.lyu@gmail.com>
...
and it moves (on-screen!) 15 pixels in the direction it is facing, 

drawing a line as it moves.  

Give it the command ``turtle.right(25)``, 
...
**龜圖** 是一種大受歡迎的教小孩寫程式的方式。

它最早是 Wally Feurzig 和 Seymour Papert 在 1966 年， 
以 Logo 程式語言開發出來的程式模組。

想像一下，有一隻龜，從 x-y 平面的座標 (0,0) 出發，
在 從 ``龜模組`` 引入 所有工具 (``from 龜模組 import`` ``*``) 之後  ，
下 指令 ``前進(15)`` ，
然後，就發現 真的有一隻龜 在螢幕上 前進 15 (單位為螢幕上的 *畫素* pixel)。
它還拉了一條線出來。
...
.. sidebar:: Turtle star, 龜星

   Turtle can draw intricate shapes using programs that repeat simple
   moves.
   
   龜 能運用程式 重複基本移動， 畫出 錯綜複雜的形狀。

   .. image:: http://docs.python.org/3.3/_images/turtle-star.png
      :align: center

   .. literalinclude:: turtle-star.py
   
   .. literalinclude:: 龜星.py
...
**程序性** (procedural) 的界面提供了 **函數** 的呼叫，這些函數是從 2 個主要的 **物類** 中的 **方法** 衍伸而來，
主要的 物類 包括了 :class:`幕類` 和 :class:`龜類`。
這些可用的函數與物類中的方法同名， 當第一個 *幕類函數* 被呼叫時，
有一個幕(物件)會產生出來，而 當第一個 *龜類函數* 被呼叫時，
有一隻(無名)龜(物件)也會自動被生出來了，
若要在一個幕上要使用多隻龜，則無法僅使用程序性的界面，那時必須運用 **物件導向** (object-oriented) 的界面了。

這個 **龜模組** 目前 *無法* 使用 *多* 個 幕。
...
Turtle motion
   Move and draw
      | :func:`forward`     | :func:`fd`     | :func:`前進`  | :func:`前`
      | :func:`backward`    | :func:`bk`     | :func:`back`  | :func:`後退` | :func:`後`
      | :func:`right`       | :func:`rt`     |  :func:`左轉` |  :func:`左`
      | :func:`left`        | :func:`lt`     | :func:`右轉`  | :func:`右`
      | :func:`goto`        | :func:`setpos` | :func:`setposition` | :func:`前往` | :func:`去到`
      | :func:`setx`
      | :func:`sety`
